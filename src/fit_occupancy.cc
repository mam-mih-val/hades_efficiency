...
int main(int n, char** args) {
  ...
  for( auto histo : pdg_delta_phi_vs_pt_rapidity ){
    ...
    std::string name = "v1_y_pT_"+std::to_string(p);
    v1_vs_pt_rapidity.push_back( new TH2F( name.c_str(), ";y;pT;v_{1}",
                                         15, -0.75, 0.75,
                                         20, 0.0, 2.0 ) );
    for( size_t y =1; y <=n_bins_y; ++y){
      for(size_t pT =1; pT <n_bins_pt; ++pT){
        name+="_"+std::to_string(y)+"_"+std::to_string(pT);
        auto proj = histo->ProjectionZ( name.c_str(),y, y+1, pT, pT+1 );
        name+="_fit";
        auto fit_func = new TF1( name.c_str(), "[0]*( 1+2*[1]*cos(x)+2*[2]*cos(2*x) )", -3.15, 3.15 );
        proj->Fit(fit_func);
